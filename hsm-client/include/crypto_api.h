...
:stub_(crypto::CryptoService::NewStub(grpc::CreateChannel(("0.0.0.0:50051"), grpc::InsecureChannelCredentials()))),userId(userId) {}
//config
...
generateRSAKeyPair(std::vector<KeyPermission> permissions);
std::pair<std::string, std::string>
generateECCKeyPair(std::vector<KeyPermission> permissions);
// sign-verify
...
CK_RV ECCencrypt(std::string keyId, void *in, size_t inLen,
                 void *out, size_t &outLen);
CK_RV ECCdecrypt(std::string keyId, void *in, size_t inLen,
                 void *out, size_t &outLen);
// rsa
size_t getRSAencryptedLength();
size_t getRSAdecryptedLength();
CK_RV RSAencrypt(std::string keyId, void *in, size_t inLen,
                 void *out, size_t outLen);
CK_RV RSAdecrypt(std::string keyId, void *in, size_t inLen,
                 void *out, size_t &outLen);
...
size_t getAESencryptedLength(size_t dataLen, const std::string &keyId, AESChainingMode chainingMode);
CK_RV AESencrypt(int recieverId, void *in, size_t inLen,
                 void *&out, unsigned int &outLen, AsymmetricFunction func,
                 AESKeyLength keyLength, AESChainingMode chainingMode,
                 std::string keyId);
CK_RV AESdecrypt(int receiverId, void *in, size_t inLen, void *&out, size_t &outLen);
...
CK_RV encrypt(int receiverId, void *in, size_t inLen, void *out,
              size_t &outLen);
