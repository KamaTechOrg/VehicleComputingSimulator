    ...
    timer(nullptr),sqlDataManager(new DbManager(this))
{
    ...
    const QString styleSheet =
        ...
        QMap<KeyPermission, bool> allPermissions = {
            {KeyPermission::VERIFY, true},
            {KeyPermission::SIGN, true},
            {KeyPermission::ENCRYPT, true},
            {KeyPermission::DECRYPT, true},
            {KeyPermission::EXPORTABLE, true}
        };
    ...
    addId(id++);
    Process *hsmProcess =
        new Process(id, "HSM", "../../hsm-server/CMakeLists.txt",
            "QEMUPlatform", allPermissions);
    addProcessSquare(
        hsmProcess,
        QPoint((id % 2) * (sizeSquare + 10), (id / 2) * (sizeSquare + 10)),
