...
grpc::Status CryptoServiceServer::encrypt(grpc::ServerContext* context, const crypto::EncryptRequest* request, crypto::EncryptResponse* response)
{
    ...
    ::encrypt(request->sender_id(), request->receiver_id(), static_cast<void*>(const_cast<char*>(request->data().data())), request->data().length(), out, outLen, signature, signatureLen, request->counter());
...
{
    size_t outLen = ::getEncryptedLen(request->sender_id(), request->encrypted_data().length(), request->isfirst());
    void* out = new uint8_t[outLen];
    ::decrypt(request->sender_id(), request->receiver_id(), static_cast<void*>(const_cast<char*>(request->encrypted_data().data())), request->encrypted_data().length(), static_cast<void*>(const_cast<char*>(request->signature().data())), request->signature().length(), out, outLen, request->counter());
                                ...
                                crypto::AESEncryptResponse *response) 
{
  size_t outLen = ::getAESencryptedLength(
      request->data().length(), true,
      static_cast<AESChainingMode>(request->chainingmode()));
  uint8_t out[outLen];
  ::AESencrypt(request->sender_id(), request->receiver_id(),
  ...
  response->set_encrypted_data(reinterpret_cast<char *>(out), outLen);
  return grpc::Status::OK;
}
 ...
 */
grpc::Status
CryptoServiceServer::AESdecrypt(grpc::ServerContext *context,
                                const crypto::AESDecryptRequest *request,
                                crypto::AESDecryptResponse *response) 
{
  size_t outLen = ::getAESdecryptedLength(request->data_in().length(), false, static_cast<AESChainingMode>(request->chainingmode()));
  uint8_t *out = new uint8_t[outLen];
  ::AESdecrypt(request->sender_id(), request->receiver_id(),
               (void *)request->data_in().data(), request->data_in().length(),
               out, outLen, static_cast<AESKeyLength>(request->key_length()),
               static_cast<AESChainingMode>(request->chainingmode()),
               request->counter(), request->key_id());
  response->set_decrypted_data(out, outLen);
  return grpc::Status::OK;
...
{
    response->set_len(::getEncryptedLen(request->senderid(), request->inlen(), true));
...
{
    response->set_len(::getDecryptedLen(request->senderid(), request->inlen(), true));
...
int main(int argc, char** argv) {
    ...
    ::configure(1, config);
    ::configure(2, config);
    ::bootSystem({{1, {  KeyPermission::VERIFY,
        KeyPermission::SIGN,
        KeyPermission::ENCRYPT,
        KeyPermission::DECRYPT,
        KeyPermission::EXPORTABLE}},{2,  {KeyPermission::VERIFY,
        KeyPermission::SIGN,
        KeyPermission::ENCRYPT,
        KeyPermission::DECRYPT,
