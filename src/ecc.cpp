...
Point convertMessageToPoint(const std::string& text) 
{
    ...
    throw std::runtime_error("Unable to convert message to a valid point on the curve.");
}
...
std::string decrypt(EncryptedMessage ciphertext)
{
    ...
    Point decrypted = add(Point(ciphertext.c2X,calculateY(ciphertext.c2X)* (ciphertext.c1Y ? -1 : 1) ), negTemp);
