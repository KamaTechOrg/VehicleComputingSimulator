 ...
 * in parallel using multiple threads in CTR (Counter) mode. Each thread is responsible
 * for encrypting a separate block.
 ...
 * @param roundKeys Pointer to the array of round keys used for AES encryption.
 * @param keyLength The length of the AES key being used.
 * @param lastData  Pointer to the initialization vector (IV) used for the counter.
 */
 ...
 * The decryption in CTR mode is similar to encryption. This function handles
 * the decryption of a single block by computing the counter value and applying
 * XOR between the encrypted counter and the ciphertext.
 * @param input     Pointer to the input block of ciphertext data (BLOCK_BYTES_LEN bytes).
 * @param output    Pointer to the output block where the decrypted plaintext will be stored.
 * @param roundKeys Pointer to the array of round keys used for AES encryption.
 * @param keyLength The length of the AES key being used.
 * @param iv        Pointer to the initialization vector (IV) used for the counter.
 * @param blockIndex The index of the current block to be decrypted.
 */
 ...
 * in parallel using multiple threads. Each thread is responsible for decrypting a separate block.
...
/**
 * Initializes the encryption process using AES in CTR mode.
 * It generates a random IV, encrypts the provided data, and stores the encrypted output.
 * @param block     Pointer to the input block of plaintext data.
...
/**
 * Continues the encryption process for subsequent blocks of data using AES in CTR mode.
 * @param block     Pointer to the input block of plaintext data.
 * @param inLen     Length of the input data.
 * @param out       Pointer to the output buffer where the encrypted data will be stored.
 * @param outLen    Length of the output buffer.
...
/**
 * Initializes the decryption process using AES in CTR mode.
 * It extracts the IV from the input, decrypts the data, and stores the plaintext output.
 * @param block     Pointer to the input block of ciphertext data.
 * @param inLen     Length of the input data.
 * @param out       Pointer to the output buffer where the decrypted data will be stored.
 * @param outLen    Reference to store the length of the decrypted data.
 * @param key       Pointer to the AES key.
 * @param keyLength The length of the AES key being used.
 */
...
/**
 * Continues the decryption process for subsequent blocks of data using AES in CTR mode.
 * @param block     Pointer to the input block of ciphertext data.
 * @param inLen     Length of the input data.
 * @param out       Pointer to the output buffer where the decrypted data will be stored.
 * @param outLen    Reference to store the length of the decrypted data.
