...
// //         if (outFile.is_open()) {
// //             it->second->serialize(outFile);  // Assuming AsymmetricKeys
// has a
// //             serialize method outFile.close();
// //         } else {
...
// //         if (inFile.is_open()) {
// //             it->second->deserialize(inFile);  // Assuming AsymmetricKeys
// has
// //             a deserialize method inFile.close();
// //         } else {
...
// //     if (inFile.is_open()) {
// //         std::string perms((std::istreambuf_iterator<char>(inFile)),
// //         std::istreambuf_iterator<char>()); permissions[keyId] = perms;
// //         inFile.close();
// //     } else {
...
// //     // Example check - customize based on your actual encryption function
// //     types return func >= SIGNATURE_SHA256_CBC && func <=
// //     ENCRYPT_256_AND_SIGN_SHA3_512_OFB;
// // }

// // void Hsm::saveKeyWithPermissionToFile(int keyID, const std::string&
// fileName)
// // {
// //     auto keyIt = Hsm::keys.find(keyID);
// //     if (keyIt == Hsm::keys.end()) {
// //         std::cerr << "Error: Key with ID " << keyID << " not found." <<
// //         std::endl; return;
// //     }

// //     auto metaDataIt = Hsm::keyMetaData.find(keyID);
// //     if (metaDataIt == Hsm::keyMetaData.end()) {
// //         std::cerr << "Error: Metadata for key with ID " << keyID << " not
// //         found." << std::endl; return;
// //     }
...
// std::map<int, std::string> Hsm::EccPublicKeysIds;
// std::map<int, std::string> Hsm::EccPrivateKeysIds;
// std::map<int, std::string> Hsm::RsaPublicKeysIds;
// std::map<int, std::string> Hsm::RsaPrivateKeysIds;
// std::map<int, std::string> Hsm::symmetricKeysIds;
// std::map<int, encryptionFunction> Hsm::encryptionFunctions;
...
// void Hsm::updateACL(int userId, const std::string &fileName,
//                     const std::vector<KeyPermission> &permissions,
//                     const std::string &keyId, const std::string &keyType) {
...
// Hsm::generateECCKeyPair(int userId, std::vector<KeyPermission> permissions) {
...
//     privFile.close();
//     std::cout << "ECC private key saved to file: " << privateKeyFile
//               << std::endl;
//   } else {
//     std::cerr << "Error: Could not open file " << privateKeyFile << " to
//     write."
//               << std::endl;
//     return {"", ""};
//   }
...
//     pubFile.close();
//     std::cout << "ECC public key saved to file: " << publicKeyFile <<
//     std::endl;
//   } else {
//     std::cerr << "Error: Could not open file " << publicKeyFile << " to
//     write."
//               << std::endl;
//     return {"", ""};
//   }
...
//   updateACL(userId, privateKeyFile, permissions, privateKeyId,
//   "ECC-Private"); updateACL(userId, publicKeyFile, permissions, publicKeyId,
//   "ECC-Public");
...
// std::string Hsm::generateAESKey(int userId,
...
//   } else {
//     std::cerr << "Error: Could not open file " << keyFileName << " to write."
//               << std::endl;
//     return "";
//   }
...
// std::pair<std::string, std::string>
// Hsm::generateRSAKeyPair(int userId, std::vector<KeyPermission> permissions) {
//   // Create unique identifiers for the keys
//   std::string privateKeyId = generateKeyId();
//   std::string publicKeyId = generateKeyId();
...
//   rsaGenerateKeys(RSA_KEY_SIZE, publicKey, pubLen, privateKey, privLen);

//   // Save private key to file
//   std::ofstream privFile(privateKeyFile, std::ios::binary);
//   if (privFile.is_open()) {
//     privFile.write(reinterpret_cast<char *>(privateKey), privLen);
//     privFile.close();
//     std::cout << "RSA private key saved to file: " << privateKeyFile
//               << std::endl;
//   } else {
//     std::cerr << "Error: Could not open file " << privateKeyFile << " to
//     write."
...
//   if (pubFile.is_open()) {
//     pubFile.write(reinterpret_cast<char *>(publicKey), pubLen);
//     pubFile.close();
//     std::cout << "RSA public key saved to file: " << publicKeyFile <<
//     std::endl;
//   } else {
//     std::cerr << "Error: Could not open file " << publicKeyFile << " to
//     write."
...
//   // Update ACL with permissions
//   updateACL(userId, privateKeyFile, permissions, privateKeyId,
//   "RSA-Private"); updateACL(userId, publicKeyFile, permissions, publicKeyId,
//   "RSA-Public");
...
// std::string Hsm::getAuthorizedKey(int userId, std::string &keyId,
...
//         if (perm.contains(permission) && perm[permission].get<bool>() ==
//         true) {
...
//       "Permission not found or not granted for this user and key.");
// }
...
// //     if (!keyFile.is_open()) {
// //         std::cerr << "Error opening key file for writing: " << fileName <<
// //         std::endl; return;
// //     }
...
// //     keyFile.close();
// // }
// // void Hsm::addKeyToAcl(int keyId, const std::string& fileName, const
// // nlohmann::json& acl) {
// //     std::string aclFileName = "../keys/keys.json";
// //     std::ifstream aclFile(aclFileName);
// //     nlohmann::json aclJson;
...
// //     if (!outFile.is_open()) {
// //         std::cerr << "Error opening ACL file for writing: " << aclFileName
// <<
// //         std::endl; return;
// //     }
...
// std::string Hsm::getPublicKeyIdByUserId(int userId, asymmetricFunctions func)
// {
//   if (func == RSA)
//     return RsaPublicKeysIds[userId];
//   else
//     return EccPublicKeysIds[userId];
// }

// std::string Hsm::getPrivateKeyIdByUserId(int userId, asymmetricFunctions
// func) {
//   if (func == RSA)
//     return RsaPrivateKeysIds[userId];
//   else
//     return EccPrivateKeysIds[userId];
// }

// static Hsm &Hsm::getInstance() {
//   static Hsm instance;
//   return instance;
// }