...
std::vector<uint8_t> createTestBuffer() {
  ...
  uint32_t status = 5;
  buffer.push_back(static_cast<uint8_t>(status >> 24)); // Most significant byte
  ...
  buffer.push_back(pressBytes[3]); // Big-endian (most significant byte first)
  ...
  int32_t flags = -5;
  buffer.push_back(static_cast<uint8_t>(flags >> 24)); // Most significant byte
  ...
  // Since we need to convert to big-endian, we add the bytes from most
...
// Helper function to create a test JSON file for bit_field with nested fields
// of int and char
...
TEST(PacketParserTest, LittleEndianBuffer) {
  ...
  EXPECT_DOUBLE_EQ(*static_cast<double *>(parser.getFieldValue("double_value")),
                   54321.9876);
...
TEST(PacketParserTest, FieldTypeDetection) {
  ...
  createTestJson(jsonFilePath);
  jsonFiles.push_back(jsonFilePath);
  PacketParser parser(jsonFilePath, nullptr); // No buffer needed for this test
