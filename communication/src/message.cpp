...
Message::Message(uint32_t srcID, void *data, int dlc, bool isBroadcast,
                 uint32_t destID)
{
    size_t size = dlc;
    uint32_t tps = (size + SIZE_PACKET - 1) /
                   SIZE_PACKET;  // Calculate the number of packets needed
    for (uint32_t i = 0; i < tps; ++i) {
        uint8_t packetData[SIZE_PACKET];
        size_t copySize = std::min(
            size - i * SIZE_PACKET,
            (size_t)
                SIZE_PACKET);  // Determine how much data to copy for each packet
        std::memcpy(packetData, (uint8_t *)data + i * SIZE_PACKET, copySize);
        uint32_t id = srcID + destID;
        packets.emplace_back(id, i, tps, srcID, destID, packetData, copySize,
                             isBroadcast, false, false);
    }
}
...
void *Message::completeData() const
{
    ...
    for (const auto &packet : packets) {
        std::memcpy(static_cast<char *>(data) + packet.header.PSN * SIZE_PACKET,
                    packet.data, packet.header.DLC);
    }
    return data;
}
