...
Communication::Communication(uint32_t id, void (*passDataCallback)(uint32_t, void *))
    : TEC(0), REC(0), state(ErrorState::Active),
      client(std::bind(&Communication::receivePacket, this,
                       std::placeholders::_1))
{
    setId(id);                             // Set the communication ID
    setPassDataCallback(passDataCallback); // Set the callback for data passing
    ...
    MessageType messageType /*= MessageType::DATA_MESSAGE*/)
{
    ...
    Message msg(srcID, destID, data, dataSize,
                messageType); // Create a new message
...
bool Communication::checkDestId(Packet &p)
{
    ...
    return p.getIsBroadcast() || p.getDestId() == id;
}

bool Communication::validCRC(Packet &packet)
{
...
{
    id = newId; // Set the new ID for the communication instance
}
...
void Communication::setPassDataCallback(void (*callback)(uint32_t, void *))
{
    ...
    passData = callback; // Set the callback function for passing data
}