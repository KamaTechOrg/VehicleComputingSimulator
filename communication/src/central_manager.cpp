...
CentralManager* CentralManager::getInstance(std::vector<int> ports, const std::vector<uint32_t>& processIds, int maxCriticalProcessID)
{
    if (instance == nullptr)
        instance = new CentralManager(ports, processIds, maxCriticalProcessID);
    
    return instance;
}

// Singleton pattern: private constructor
CentralManager::CentralManager(std::vector<int> ports, const std::vector<uint32_t>& processIds, int maxCriticalProcessID) :ports(ports), running(false)
{
...
ErrorCode CentralManager::updateProcessID(const uint32_t processID, const uint32_t port, bool isConnected)
{
    ...
    // if(isConnected)
    //     sync.registerProcessManager(processID);
    return ErrorCode::SUCCESS;
}
